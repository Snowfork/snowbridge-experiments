...
import { Blake2bConsenSys } from "../types"
// import blake2TestVectors from "./blake2-kat.json"

describe("Blake2bConsensys", function () {
  let blake2b: Blake2bConsenSys

  beforeEach(async function () {
    const blake2bFactory = await ethers.getContractFactory("Blake2bConsenSys")
    blake2b = (await blake2bFactory.deploy()) as Blake2bConsenSys
  ...
  describe("hash", function () {
    it.only("should work", async function () {
      ...
      expect(
        (await blake2b["blake2b(bytes,bytes,uint64)"](input, "0x", 64)).reduce(
          (s, e) => s + e.toHexString().slice(2),
          ""
        )
      ).to.equal(expected)
    })
