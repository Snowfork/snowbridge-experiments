...
import { MMRVerification } from "../types"

async function testFixture() {
    const mmrVerificationFactory = await ethers.getContractFactory("MMRVerification")
    const mmrVerificationContract = (await mmrVerificationFactory.deploy()) as MMRVerification
    await mmrVerificationContract.deployed()

    return { mmrVerificationContract }
}

describe("MMRVerification Contract", function () {
    context("constructor", function () {
      it("Should deploy the contract successfully", async function () {
        const { mmrVerificationContract } = await testFixture()

        expect(mmrVerificationContract).to.haveOwnProperty("address")
      })
    })

    context("7-leaf, 11-node MMR", function () {
        let mmrVerification: MMRVerification;
        ...
        beforeEach(async function () {
            const { mmrVerificationContract } = await testFixture()
            mmrVerification = mmrVerificationContract;
        })
        ...
        it('should verify valid proof for leaf index 0 (node 1)', async function () {
          ...
          expect(await mmrVerification.callStatic.verifyInclusionProof(root, leafNodeHash, proof.leaf_index, proof.leaf_count, proof.items)).to.be.true
        });
        ...
        it('should not verify invalid proofs', async () => {
            ...
            expect(await mmrVerification.callStatic.verifyInclusionProof(root, leafNodeHash, proof.leaf_index, proof.leaf_count, proof.items)).to.be.false
        });
    })

    context("15-leaf, 26-node MMR", function () {
        before(function() {
